...
Status: ARCH-FIX4 WIP normative design policy; local gates are available, but checkpoint `04351799ad780602bf73324336f40ab300c80323` is not an accepted baseline and independent ARCH-REVIEW6 remains required
...
ARCH-FIX1 through ARCH-FIX3 used dependency-free subset checks. SCHEMA-VALIDATION1 subsequently installed the exact development-only Draft 2020-12 validator and lock file described below. XOBJ-GRAPH1 made all 18 registered cross-object rules executable over fixed synthetic materialized graphs. ARCH-REVIEW5 nevertheless reproduced two false gate results: XOBJ-011 universally imposed collector/pass execution binding on non-collector consent variants, and digest completeness used replaceable counts. ARCH-FIX4 narrows those two blockers; CI integration and independent ARCH-REVIEW6 remain later work. No local gate is a runtime dependency, provenance proof, compatibility experiment, production-readiness claim, or declaration that the schemas are production-ready.
...
XOBJ graph fixtures are `FullContractInstance` vectors whose constituent objects all pass the current contract schemas before graph evaluation. That precondition does not label those schemas production-ready. The graph wrapper itself is deliberately test-only and has no production schema. Its materializer accepts only bounded data mutations, rejects unknown object kinds and absolute paths, and returns a deeply frozen graph. Missing required XOBJ input is `XOBJ_GRAPH_INPUT_MISSING`; a normative violation uses the exact XOBJ error code from `integrity-model.md`.
...
XOBJ-011 additionally dispatches all nine consent types into the immutable models `CollectorExecution`, `Export`, `Remediation`, and `Reboot`. Exit code 0 requires `required_consent_variants == executable_consent_variants == executed_consent_variants == positively_covered_consent_variants`, complete required negative/substitution coverage of the same set, empty uncovered/skipped sets, and exact required vector IDs. Export, Remediation, and Reboot positives omit collector/execution binding; DefenderOffline separately proves the existing collector-scoped model. Schema-required binding members are not removed merely to create application negatives: “missing” XOBJ vectors retain valid local shape while making the cross-object target/action/digest/workflow binding unresolvable or mismatched.

## JCS-DIGEST1 development gate

The digest command executes permanent JSON-compatible RFC 8785, Unicode, number, ProfileDigest, FileDigest, and ContentDigest fixtures plus direct in-memory cases that JSON files cannot represent. Direct cases cover undefined, functions, symbols, BigInt, non-finite numbers, lone surrogates, sparse arrays, cycles, accessors, proxies, symbol keys, typed arrays, and non-plain objects. Every rejection is controlled, every ProfileDigest fixture is nonempty and schema-valid, every FileDigest positive has an exact mutation digest, and separate ContentDigest cases cover BOM-only, LF-only, mixed line endings, preserved NUL, Unicode non-normalization, and invalid binary UTF-8.

Exit code 0 no longer treats counts as completeness proof. Immutable required-ID sets cover catalog digest cases, JCS conformance, direct positives, direct negatives, ProfileDigest, FileDigest, ContentDigest, schema format/keyword/namespace guards, and checked arithmetic. Every ID must exist once in the correct category, execute, remain unskipped, and meet its expectation. Every `npm test` mutates every required ID six ways in memory: delete, replace by a duplicate, duplicate directly, change category, skip, and do not execute. Controlled failures use `DIGEST_REQUIRED_CASE_MISSING`, `DIGEST_REQUIRED_CASE_DUPLICATE`, `DIGEST_REQUIRED_CASE_NOT_EXECUTED`, `DIGEST_REQUIRED_CASE_SKIPPED`, `DIGEST_CASE_CATEGORY_MISMATCH`, and `DIGEST_REQUIRED_SET_MISMATCH`.

JSON output includes `required_case_ids_by_category`, `actual_case_ids_by_category`, missing/duplicate/category-mismatch/skipped/not-executed ID arrays, and `required_set_complete`. Optional embedded checks are explicitly additional. RFC wording is limited to `rfc8785_conformance_status: covered_conformance_set_passed`, `conformance_claim: CoveredSet`, `covered_case_count`, `full_corpus_claimed: false`, and the official/derived covered vectors. Locale-aware sorting, environmental configuration, network retrieval, and generated report files are outside the gate.
...
Passing SCHEMA-VALIDATION1 is a prerequisite for architecture baseline acceptance, not acceptance itself. ARCH-FIX4 still requires independent ARCH-REVIEW6. The local gates establish validation behavior for the current synthetic design contracts; they do not establish runtime conformance, provenance, Velociraptor compatibility, production readiness, or production-ready schemas.
